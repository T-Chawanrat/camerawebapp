...
//   // 🔍 state สำหรับระบบค้นหา S/N → REFERENCE
...
// 👉 type response จาก backend getBillsBySerial
type BillsBySerialResponse = {
  success: boolean;
  data?: {
    REFERENCE: string;
    SERIALS: string[];
    bill: {
      ...
      SERIAL_NO: string;
      REFERENCE: string;
      // field อื่น ๆ มีได้ แต่ไม่จำเป็นต้องใส่หมดตอนนี้
    };
    serialCount: number;
  };
  message?: string;
  ...
  const handleSearchSerial = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${apiUrl}/serial?SERIAL_NO=${encodeURIComponent(serial)}`
      );

      if (!res.ok) {
        // พยายามอ่าน message จาก backend ถ้ามี
        let msg = "ค้นหาข้อมูลไม่สำเร็จ";
        try {
          const errJson = (await res.json()) as Partial<BillsBySerialResponse>;
      ...
      const json: BillsBySerialResponse = await res.json();

      if (!json.success || !json.data) {
        setError(json.message || "ไม่พบข้อมูลสำหรับ S/N นี้");
        return;
      }

      setReference(json.data.REFERENCE);

      // ป้องกันกรณี SERIALS ซ้ำ → unique
      const uniqueSerials = Array.from(new Set(json.data.SERIALS));
      setSerialsInReference(uniqueSerials);
    } catch (e) {
      console.error("Search serial error:", e);
      setError(
        e instanceof Error
          ? e.message || "ค้นหาข้อมูลไม่สำเร็จ กรุณาลองใหม่อีกครั้ง"
          : "ค้นหาข้อมูลไม่สำเร็จ กรุณาลองใหม่อีกครั้ง"
      );
    } finally {
      setSearchingSerial(false);
    }
        ...
        {reference && (
          <div className="mt-3 p-3 border rounded-md bg-gray-50 text-sm">
            ...
            {serialsInReference.length > 0 && (
              <div className="mt-2">
                <p className="font-semibold mb-1">
                  S/N ทั้งหมดใน REF นี้{" "}
                  <span className="text-brand-700">
                    ({serialsInReference.length} รายการ)
                  </span>
                </p>
